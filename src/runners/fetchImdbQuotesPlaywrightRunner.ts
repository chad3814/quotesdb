...
import { getIMDbPlaywrightClient } from '@/lib/imdbPlaywright';
import { parsePlaywrightQuotes } from '@/lib/imdbPlaywrightParser';
...
export async function run(payload: FetchImdbQuotesPayload): Promise<FetchImdbQuotesResult> {
  ...
  logger.info(`Starting Playwright IMDb quote fetch for movie ${movieId} (IMDb: ${imdbId})`);
  ...
  const playwrightClient = getIMDbPlaywrightClient();
  ...
  try {
    ...
    // Fetch quotes from IMDb using Playwright
    const rawQuotes = await playwrightClient.fetchAllQuotes(imdbId);
    ...
    result.totalQuotesFound = rawQuotes.quotes.length;
    logger.info(`Found ${result.totalQuotesFound} total quotes on IMDb page`);

    // Parse the raw quotes
    const parsedQuotes = parsePlaywrightQuotes(rawQuotes);
    ...
    logger.info(`Playwright IMDb quote import completed for movie ${movieId}`, result);
    return result;

  } catch (error) {
    logger.error(`Failed to fetch IMDb quotes with Playwright for movie ${movieId}`, {
      error: error instanceof Error ? error.message : String(error),
      movieId,
      imdbId
    });
    ...
    // Don't close the client here as it's a singleton that might be reused
