...
export default function AuthModal({ isOpen, onClose, onSuccess }: AuthModalProps) {
  ...
  const handleCancel = () => {
    setError("Authentication was cancelled. You'll need to sign in to add quotes.")
    setLoading(null)
    setTimeout(() => {
      setError(null)
      onClose()
      ...
      <div className="bg-white rounded-lg p-6 w-full max-w-md mx-4">
        ...
        <div className="space-y-3">
          ...
          <button
            ...
            disabled={!!loading}
            className="w-full flex items-center justify-center px-4 py-2 border border-gray-300 rounded-md shadow-sm bg-black text-sm font-medium text-white hover:bg-gray-800 disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {loading === "apple" ? (
              "Signing in..."
            ) : (
