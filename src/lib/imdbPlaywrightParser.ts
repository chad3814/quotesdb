...
import { PlaywrightQuoteData } from '@/lib/imdbPlaywright';
import { ParsedQuote, ParsedQuoteLine } from '@/lib/imdbQuoteParser';

export function parsePlaywrightQuotes(rawQuotes: PlaywrightQuoteData): ParsedQuote[] {
  const parsedQuotes: ParsedQuote[] = [];

  if (!rawQuotes?.quotes || !Array.isArray(rawQuotes.quotes)) {
    logger.warn('Invalid Playwright quote data structure');
    return parsedQuotes;
  }
  ...
  for (const quote of rawQuotes.quotes) {
    ...
    try {
      ...
      logger.error('Error parsing Playwright quote', {
  ...
  logger.info(`Successfully parsed ${parsedQuotes.length} Playwright quotes`);
  return parsedQuotes;
}
